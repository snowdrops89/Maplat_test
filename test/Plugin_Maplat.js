    ...
    fn: function (lineData, sys) {
      ...
      if(Object.prototype.toString.call(lineData) === '[object Array]'){
        ...
        if (sys.__v0['Maplat線種類'] == "点線") {
          lineData["style"]["stroke"]["lineCap"]="butt"
          lineData["style"]["stroke"]["lineDash"]=[2*w, 2*w, 2*w, 2*w]
        } else if (sys.__v0['Maplat線種類'] == "破線") {
          lineData["style"]["stroke"]["lineCap"]="butt"
          lineData["style"]["stroke"]["lineDash"]=[4*w, 2*w, 4*w, 2*w]
        } else if (sys.__v0['Maplat線種類'] == "一点鎖線") {
          lineData["style"]["stroke"]["lineCap"]="butt"
          lineData["style"]["stroke"]["lineDash"]=[6*w, 2*w, 2*w, 2*w]
        } else if  (sys.__v0['Maplat線種類'] == "丸点線") {
          lineData["style"]["stroke"]["lineCap"]="round"
          lineData["style"]["stroke"]["lineDash"]=[1, 2*w, 1, 2*w]
        }
      }
      sys.__v0['MaplatApp'].addVector(lineData)
    }
  },
